...
$(call USER_VARIABLE,QEMUFLAGS,-m 2G -d int -no-reboot -no-shutdown -serial stdio -cpu max)

override IMAGE_NAME := Nyaux-$(KARCH)
...
.phony: run-kvm-gdb
run-kvm-gdb: run-$(KARCH)-kvmdebug
.phony: run-gdb
run-gdb: run-$(KARCH)-debug
.PHONY: run-hdd
...
run-x86_64: ovmf/ovmf-code-$(KARCH).fd ovmf/ovmf-vars-$(KARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(KARCH) \
		...
		$(QEMUFLAGS)
run-x86_64-kvm: ovmf/ovmf-code-$(KARCH).fd ovmf/ovmf-vars-$(KARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(KARCH) \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=ovmf/ovmf-code-$(KARCH).fd,readonly=on \
		-drive if=pflash,unit=1,format=raw,file=ovmf/ovmf-vars-$(KARCH).fd \
		...
		-cpu host \
		$(QEMUFLAGS)
run-x86_64-kvmdebug: ovmf/ovmf-code-$(KARCH).fd ovmf/ovmf-vars-$(KARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(KARCH) \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=ovmf/ovmf-code-$(KARCH).fd,readonly=on \
		-drive if=pflash,unit=1,format=raw,file=ovmf/ovmf-vars-$(KARCH).fd \
		...
		-cpu host \
		$(QEMUFLAGS)
run-x86_64-debug: ovmf/ovmf-code-$(KARCH).fd ovmf/ovmf-vars-$(KARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(KARCH) \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=ovmf/ovmf-code-$(KARCH).fd,readonly=on \
		-drive if=pflash,unit=1,format=raw,file=ovmf/ovmf-vars-$(KARCH).fd \
...
$(IMAGE_NAME).iso: limine/limine kernel
	...
	cp -v kernel/NyauxKT iso_root/boot/
