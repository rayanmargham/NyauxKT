...
extern crate alloc;
use crate::{mem::HHDM, print, println};
use alloc::boxed::Box;
...
impl KernelApi for KTUACPIAPI {
    ...
    unsafe fn io_map(
        ...
        let bro = alloc::alloc::alloc(Layout::new::<IOthingy>()).cast::<IOthingy>();
        unsafe {
            (*bro).base = base;
            (*bro).len = len;
        }
    ...
    unsafe fn io_read(
        ...
        self.raw_io_read(IOAddr::new((*ok).base.as_u64() + offset as u64), byte_width)
    }
    ...
    unsafe fn io_write(
        ...
        let ok = handle.as_u64();
        let ok = core::ptr::with_exposed_provenance_mut::<IOthingy>(ok as usize);
        println!("{:#x}", (*ok).base.as_u64());
        self.raw_io_write(
            IOAddr::new((*ok).base.as_u64() + offset as u64),
            byte_width,
            val,
        )
    }
    ...
    unsafe fn raw_io_read(
        ...
        if byte_width.is_power_of_two() == false {
    ...
    let st = uacpi::init(
        PhysAddr::new(
            rsdp.get_response().unwrap().address() as u64
                - HHDM.get_response().unwrap().offset() as u64,
        ),
        LogLevel::INFO,
        false,
    );
