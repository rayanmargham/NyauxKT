...
unsafe impl Sync for KTNode {}
unsafe impl Send for KTNode {}
static HEAD: Mutex<KTNode> = Mutex::new(KTNode { next: None });
...
impl slab_header {
    fn init(size: usize) -> NonNull<Self> {
        ...
        let it = NonNull::new(unsafe {
            data.add(HHDM.get_response().unwrap().offset() as usize) as *mut slab_header
        })
        .unwrap();
        unsafe {
